...
const leadBehaviorLines = [
  "Run selected = checked eligible leads only.",
  "Export = selected rows, or filtered rows when nothing is selected.",
  "Completed non-dormant leads are skipped until revival rules apply.",
];
...
const Leads = () => {
  ...
  const [leadGuidePopover, setLeadGuidePopover] = useState(null); // "behavior" | status string | null
  ...
  return (
    <section className="app-surface-card p-3 sm:p-4">
      ...
      <div className="mb-3">
        <GuidePanel
          title="Lead guide"
          info="Workflow statuses (New, Active, HITL, …), bulk run-selected behaviour, and CSV export rules. Line-by-line meanings are in the expanded panel."
        >
          <div className="relative flex flex-wrap items-center gap-x-2 gap-y-2">
            <div className="relative flex items-center gap-1">
              <span className="text-[11px] font-semibold text-gray-700 dark:text-volt-text">Bulk / export</span>
              <button
                type="button"
                aria-label="Run and export behavior"
                onClick={() => setLeadGuidePopover((v) => (v === "behavior" ? null : "behavior"))}
              ...
              {leadGuidePopover === "behavior" ? (
                <>
                  <div className="fixed inset-0 z-30" onClick={() => setLeadGuidePopover(null)} />
                  <div className="absolute left-0 top-7 z-40 w-64 rounded-2xl border border-gray-100 bg-white p-3 shadow-lg dark:border-volt-borderSoft dark:bg-volt-card">
                    <p className="mb-2 text-[11px] font-semibold text-gray-800 dark:text-white">Run &amp; export</p>
                    <ul className="space-y-1.5 text-[11px] text-gray-600 dark:text-volt-muted2">
                      {leadBehaviorLines.map((line) => (
            ...
            <span className="hidden h-4 w-px bg-gray-200 sm:inline dark:bg-white/15" aria-hidden />

            {leadStatusLegend.map(([status, detail]) => (
              <div key={status} className="relative flex items-center gap-0.5">
                <span className="rounded-full border border-gray-100 bg-gray-50 px-2 py-0.5 text-[11px] font-semibold text-gray-800 dark:border-volt-borderSoft dark:bg-white/10 dark:text-white">
                  {status}
                </span>
                <button
                  type="button"
                  aria-label={`${status} status definition`}
                  onClick={() => setLeadGuidePopover((v) => (v === status ? null : status))}
                  className="inline-flex h-5 w-5 flex-none items-center justify-center rounded-full border border-gray-200 bg-white text-gray-400 transition hover:border-indigo-200 hover:text-indigo-500 dark:border-volt-borderSoft dark:bg-volt-card/60 dark:text-volt-muted2 dark:hover:text-indigo-300"
                >
                  <svg viewBox="0 0 24 24" fill="none" className="h-3 w-3" aria-hidden="true">
                    <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" />
                    <path d="M12 16v-4M12 8h.01" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                ...
                {leadGuidePopover === status ? (
                  <>
                    <div className="fixed inset-0 z-30" onClick={() => setLeadGuidePopover(null)} />
                    <div className="absolute left-0 top-7 z-40 w-56 rounded-2xl border border-gray-100 bg-white p-3 shadow-lg dark:border-volt-borderSoft dark:bg-volt-card">
                      <p className="text-[11px] font-semibold text-gray-800 dark:text-white">{status}</p>
                      <p className="mt-1.5 text-[11px] text-gray-600 dark:text-volt-muted2">{detail}</p>
                    </div>
                  </>
                ) : null}
              </div>
