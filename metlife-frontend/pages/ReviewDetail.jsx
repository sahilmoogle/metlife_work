...
const ReviewDetail = () => {
  ...
  if (loading) {
    return (
      <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
  ...
  if (loadError) {
    return (
      <section className="rounded-2xl border border-amber-100 bg-amber-50/80 p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
        ...
        <div className="mt-3">
          <Link to="/reviews" className="text-xs font-semibold text-gray-600 hover:underline">
            ← Back to queue
          </Link>
        </div>
      </section>
    ...
    <section className="space-y-3">
      <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-sm font-semibold text-[#1e2a52]">{gateTitle}</h2>
            <p className="mt-1 text-[11px] text-gray-500">Thread: {data.thread_id}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-2xl border border-gray-100 bg-white p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
          ...
          <div className="mt-3 flex items-center gap-3 rounded-xl bg-indigo-50/60 p-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-gray-800">{leadName}</p>
              <p className="truncate text-xs text-gray-400">Lead ID: {data.lead_id}</p>
            </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-gray-100 pt-3 text-xs">
            <div>
              ...
              <p className="mt-1 font-medium text-gray-700">{data.suggested_persona || "—"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 font-medium text-gray-700">{(data.engagement_score ?? 0).toFixed(2)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 font-medium text-gray-700">{data.gate_type}</p>
            </div>
            <div className="col-span-2">
              <p className="text-[11px] text-gray-400">Mode</p>
              <p className="mt-1 font-medium text-gray-700">{data.review_status}</p>
            </div>
            {data.persona_confidence != null ? (
              <div className="col-span-2">
                <p className="text-[11px] text-gray-400">Persona confidence</p>
                <p className="mt-1 font-medium text-gray-700">{data.persona_confidence.toFixed(2)}</p>
              </div>
            ) : null}
            {data.handoff_briefing ? (
              <div className="col-span-2">
                <p className="text-[11px] text-gray-400">Handoff briefing</p>
                <p className="mt-1 whitespace-pre-wrap font-medium text-gray-700">{data.handoff_briefing}</p>
              </div>
            ) : null}
          </div>
        </aside>
        ...
        <article className="rounded-2xl border border-gray-100 bg-white p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold text-[#1e2a52]">Draft preview</p>
          ...
          <div className="mt-3 rounded-xl bg-gray-50 p-4 text-xs text-gray-700">
            {!editMode ? (
              <>
                <p className="font-semibold text-gray-700">Subject: {data.draft_subject || "—"}</p>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 h-9 w-full rounded-lg border border-gray-200 bg-white px-3 text-xs text-gray-800 outline-none focus:border-indigo-300"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={8}
                    className="mt-1 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-800 outline-none focus:border-indigo-300"
          ...
          <div className="mt-4 border-t border-gray-100 pt-3">
            <p className="text-xs font-semibold text-[#1e2a52]">Reviewer notes</p>
            <textarea
              value={reviewerNotes}
              onChange={(e) => setReviewerNotes(e.target.value)}
              rows={3}
              className="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-800 outline-none focus:border-indigo-300"
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <button
          ...
          className="inline-flex h-9 items-center gap-2 rounded-full border border-gray-200 bg-white px-5 text-xs font-semibold text-gray-600 hover:border-rose-200 hover:text-rose-700 disabled:opacity-60"
        >
          Reject
        </button>
        <button
          ...
          className="inline-flex h-9 items-center gap-2 rounded-full border border-gray-200 bg-white px-5 text-xs font-semibold text-gray-600 hover:border-amber-200 hover:text-amber-700 disabled:opacity-60"
        >
          Hold
        </button>
        {editMode ? (
